...
class BaseServer():
    def __init__(self, n: int, m: int, k: int, varepsilon: float, iterations: int, round: int,
                 clients: List = [], C_truth: List = [], privacy_mechanism_type: List = "GRR",
                 evaluate_type: str = "F1",
    ...
    def predict_heavy_hitters(self, stop_iter=-1) -> Dict:
        ...
        for i in range(1, self.iterations + 1):
            ...
            for client_id in range(participants, end_participants):
                ...
                privacy_module = PrivacyModule(self.varepsilon, A_i, type=self.privacy_mechanism_type, s_i=s_i,
                                               required_bits=delta_s, client_id=client_id)
