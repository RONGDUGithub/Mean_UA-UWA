...
def run_repeated_mechanisms(file_name):
    # 定义基础机制列表，每组重复不同次数
    mechanisms_list = [
        ['duchi', 'sw'],
        ['duchi', 'laplace', 'piecewise', 'sw'],
        ['duchi', 'laplace', 'piecewise', 'sw', 'duchi', 'sw'],
        ['duchi', 'laplace', 'piecewise', 'sw', 'duchi', 'laplace', 'piecewise', 'sw'],
        ['duchi', 'laplace', 'piecewise', 'sw', 'duchi', 'laplace', 'piecewise', 'sw',
         'duchi', 'laplace', 'piecewise', 'sw'],
        ['duchi', 'laplace', 'piecewise', 'sw', 'duchi', 'laplace', 'piecewise', 'sw',
         'duchi', 'laplace', 'piecewise', 'sw', 'duchi', 'laplace', 'piecewise', 'sw']
    ...
    # 转换为numpy数组
    MSE_matrix = np.array(MSE_matrix, dtype=object)
    time_matrix = np.array(time_matrix, dtype=object)

    # 保存结果到文件
    base_name = os.path.splitext(file_name)[0]
    output_filename = f'./group4/Group4_{base_name}_A.txt'
    ...
    # return MSE_matrix, time_matrix, mechanisms_list, epsilons_list


# 使用示例：
file_name = 'Betawave.xlsx'
run_repeated_mechanisms(file_name)
