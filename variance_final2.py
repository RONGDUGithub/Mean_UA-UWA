...
def Variance_sr(t, epsilon):
    # 确保 epsilon 和 t 都是 tensor
    if not isinstance(epsilon, torch.Tensor):
        epsilon = torch.tensor(epsilon, device='cuda', dtype=torch.float)
    if not isinstance(t, torch.Tensor):
        t = torch.tensor(t, device='cuda', dtype=torch.float)
...
def Variance_pm(t, epsilon):
    # 确保输入是tensor
    if not isinstance(epsilon, torch.Tensor):
        epsilon = torch.tensor(epsilon, device='cuda')
    if not isinstance(t, torch.Tensor):
...
def Variance_laplace(t, epsilon):
    # 确保输入是tensor
    if not isinstance(epsilon, torch.Tensor):
        epsilon = torch.tensor(epsilon, device='cuda', dtype=torch.float64)
    if not isinstance(t, torch.Tensor):
        t = torch.tensor(t, device='cuda', dtype=torch.float64)
